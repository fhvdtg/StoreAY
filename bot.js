...
client.on("message",message => {
          ...
          {
            if(message.author.id !== message.guild.owner.id && !message.member.hasPermission("ADMINISTRATOR")) return message.reply("**NOT ALLOWED!**");
            let channel = message.guild.channels.get(message.content.split(" ")[1]) || message.mentions.channels.first();
            let username = message.content.split(" ")[2];
            let password1 = message.content.split(" ")[3];
            message.delete(250)
            if(client.locker[username]) return message.reply("**USERNAME REAPETED!**").then(m => {m.delete(4000)});;
            if(!channel || !username || !password1 || username == "" || password1 == "")
              return message.reply("**SYNTAX: !lock #channel username password**").then(m => {m.delete(4000)});
            client.locker[username] = {
              password: password1,
              chanid: channel.id
            }
            fs.writeFileSync("./locker.json",JSON.stringify(client.locker, null, 4), err => {
              if(err) throw err;
            })
            ...
            PASSWORD: ${password1}
            `)
            .setFooter("ALPHA CODES","https://images-ext-2.discordapp.net/external/h8dBcPKgNnet5C_-Fml2qOLA-SukRxnv2xtc1ggvyE4/%3Fsize%3D1024/https/cdn.discordapp.com/icons/691882846557700107/35efe93420f724b6665f811beb0b6851.png");
            message.author.send(embed);
       
          }else if(message.content.toLowerCase().split(" ")[0] == "!login")
          {
            let username = message.content.split(" ")[1];
            let password1 = message.content.split(" ")[2];
            message.delete(250);
            if( !username || !password1 || username == "" || password1 == "")
              return message.reply("**SYNTAX: !login #username password**");
            if(!client.locker[username])
              return message.reply("**INVALID USERNAME**").then(m => {m.delete(4000)});
            if(client.locker[username].password !== password1)
              return message.reply("**INVALID PASSWORD**").then(m => {m.delete(4000)});
            let chan = message.guild.channels.get(client.locker[username].chanid);
            chan.overwritePermissions(message.author.id, {
              READ_MESSAGES: true,
              SEND_MESSAGES: true
            })
          }else if(message.content.toLowerCase().split(" ")[0] == "!nlock")
          {
            if(message.author.id !== message.guild.owner.id && !message.member.hasPermission("ADMINISTRATOR")) return message.reply("**NOT ALLOWED!**");
            let channel = message.guild.channels.get(message.content.split(" ")[1]) || message.mentions.channels.first();
            let username = message.content.split(" ")[2];
            ...
            message.delete(250);
            if( !username || !password1 || username == "" || password1 == "")
              return message.reply("**SYNTAX: !ock #channel username password**");
            if(!client.locker[username])
              return message.reply("**INVALID USERNAME**").then(m => {m.delete(4000)});
            if(client.locker[username].password !== password1)
              return message.reply("**INVALID PASSWORD**").then(m => {m.delete(4000)});
